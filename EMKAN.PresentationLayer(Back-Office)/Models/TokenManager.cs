...
namespace EMKAN.PresentationLayer_Back_Office_.Models
{
    public class TokenManager
    {
            ...
            TokenManager(EMKANDbContext db)
            {
                _db = db;
            }
            ...
            public string GenerateToken(string username)
            {
                ...
                try
                {
                    var roleList = _db.Roles.FromSqlRaw("EXEC UserPrivilege @username = {0}", username).ToList();

                    // var roleList = new EMDbContext().UserPrivilege(username).ToList();
                    //var roleList = _db.UserPrivilege(username).ToList();
